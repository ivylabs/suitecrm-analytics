    ...
    defaults: {
      buttons:[{
        ...
        action: function(tgt, st, opt) {
          var indexFav = BSRecentFilesComponent.getIndexByPath(st.compoment.favoritesRawData, st.value);
          var indexRec = BSRecentFilesComponent.getIndexByPath(st.compoment.recentRawData, st.value);
          var newArrayFavorites = $.extend([],st.compoment.favoritesRawData);
          if(indexFav == -1){ // Add to favorites
            newArrayFavorites.push(st.compoment.recentRawData[indexRec]);
          ...
          jQuery.ajax({
               ...
               success: function(result) {
                          if(result!=undefined && result!=""){
                            ...
                            st.compoment.favoritesRawCDAData = BSRecentFilesComponent.convertRecentToCDADatasource(st.compoment.favoritesRawData);
                          }
                          if(indexFav == -1){
      ...
      },
      {
        ...
        action: function(tgt, st, opt) {
          var indexRec = BSRecentFilesComponent.getIndexByPath(st.compoment.recentRawData, st.value);
          var newArrayRecent = $.extend([],st.compoment.recentRawData);
          newArrayRecent.splice(indexRec,1);
          jQuery.ajax({
               url: webAppPath+"/api/user-settings/recent",
               method: 'POST',
               data: JSON.stringify(newArrayRecent),
               success: function(result) {
                          if(result!=undefined && result!=""){
                            st.compoment.recentRawData = JSON.parse(result);
                            st.compoment.recentRawCDAData = BSRecentFilesComponent.convertRecentToCDADatasource(st.compoment.recentRawData);
                            st.compoment.update();
                          }
                        },
               async:   false
...
Dashboards.registerAddIn("bsRecents", "colType", new AddIn(RecentAddIns.addRemoveFavorites));
Dashboards.registerAddIn("bsRecents", "colType", new AddIn(RecentAddIns.link));

 
var BSRecentFilesComponent = BaseComponent.extend({
  ...
  update: function() {
    ...
    jQuery.ajax({
         ...
         success: function(result) {
                    if(result!=undefined&&result!=""){
                      myself.recentRawData=JSON.parse(result);
                      myself.recentRawCDAData = BSRecentFilesComponent.convertRecentToCDADatasource(myself.recentRawData, myself.valueProperties.showFullPath, 
    ...
    jQuery.ajax({
         ...
         success: function(result) {
                    if(result!=undefined&&result!=""){
                      ...
                      myself.favoritesRawCDAData = BSRecentFilesComponent.convertRecentToCDADatasource(myself.favoritesRawData);
                    }
                  },
         async:   false
    });
    ...
    if(myself.displayType=="table"){
      ...
      if(typeof cd.colHeaders === "undefined" || cd.colHeaders.length == 0)
        cd.colHeaders = myself.recentRawCDAData.metadata.map(function(i){return i.colName});

      if(typeof cd.colTypes === "undefined" || cd.colTypes.length == 0)
        cd.colTypes = myself.recentRawCDAData.metadata.map(function(i){return i.colType.toLowerCase()});
      
      var dtData = BSRecentFilesComponent.getDataTableOptions(cd);
       ...
       */
      if (myself.recentRawCDAData!==undefined&&myself.recentRawCDAData) {
        if((typeof(myself.postFetch)=='function')){
          myself.recentRawCDAData = myself.postFetch(myself.recentRawCDAData);
        }
        dtData.aaData = myself.recentRawCDAData.resultset;
      }
      ...
      myself.ph.find ('table').bind('click',function(e) {
        if (typeof cd.clickAction === 'function' || myself.expandOnClick) { 
          var state = {},
            target = $(e.target),
            results = myself.recentRawCDAData; 
          ...
          state.rawData = myself.recentRawCDAData;
          ...
          if(!foundAddIn && cd.colFormats) {
            ...
            if(position && typeof position[0] == "number"){
              var rowIdx = position[0],
                ...
                value = myself.recentRawCDAData.resultset[rowIdx][colIdx];
  ...
  getDataTableOptions : function(options) {
    ...
    dtData.iDisplayLength = false;
    dtData.bLengthChange = false;
    dtData.bPaginate = false;
  ...
  convertRecentToCDADatasource: function(data, showFullPath, showTitle, showLastExec, dateLastExecFormat){
    var result = {
      metadata:[],
